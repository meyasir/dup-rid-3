package com.darewrorider.Adapters;
...
public class NewOrdersListViewAdapter extends BaseAdapter {
    ...
    public NewOrdersListViewAdapter(Context context,
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            view = inflater.inflate(R.layout.adapter_new_orders, null);
        ...
        if(orders.get(position).getAcknowledge()==null||orders.get(position).getAcknowledge().matches("null")
                ||orders.get(position).getAcknowledge().matches("0000-00-00 00:00:00"))
            holder.status.setText("New Order");
        else if(orders.get(position).getPick()==null||orders.get(position).getPick().matches("null")
                ||orders.get(position).getPick().matches("0000-00-00 00:00:00"))
            holder.status.setText("Ready to Pick");
        else
            holder.status.setText("To be deliver");
        view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                onViewClick.onViewClick(position, orders.get(position).getId(),orders.get(position).getCustomerName(),
                        orders.get(position).getDetail(),orders.get(position).getDeliveryCharges(),
                        orders.get(position).getPickAddress(),orders.get(position).getDropAddress(),
                        orders.get(position).getAcknowledge(),orders.get(position).getPick());
            }
        });
        return view;
    }
    ...
    public interface OnViewClick{
        public abstract void onViewClick(int position, int id, String name,
                                         String detail, String charges,
                                         String pickAddress, String drop, String ack, String pick);
    }
}
